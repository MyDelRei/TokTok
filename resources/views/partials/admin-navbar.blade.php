<header class="bg-white shadow-sm">
    <div class="flex items-center justify-between bg-white h-16 border-b border-gray-200 px-4 md:px-8">
        <!-- Logo + Search -->
        <div class="flex items-center gap-4 md:gap-6 flex-1">
            ...
            <div class="relative flex-1 max-w-md">
                ...
                <input
                    ...
                    onkeydown="if(event.key === 'Enter'){ performGlobalRedirectSearch(this.value) }"
                />
            </div>

            <!-- Global Search Overlay -->
            <div id="globalSearchOverlay" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex justify-center items-start pt-20">
                <div class="bg-white w-full max-w-3xl rounded-lg shadow-lg p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-xl font-semibold">Global Search</h3>
                        <button onclick="closeGlobalSearch()" class="text-gray-500 hover:text-gray-800">&times;</button>
                    </div>
                    <input 
                        type="text" 
                        id="globalSearchInput" 
                        placeholder="Type to search users, books, borrow records..." 
                        class="w-full border p-2 rounded-md mb-4"
                        oninput="performGlobalSearch(this.value)"
                        onkeydown="if(event.key === 'Enter'){ 
                            const firstItem = document.querySelector('#globalSearchResults div'); 
                            if(firstItem) firstItem.click();
                        }"
                    />
                    <div id="globalSearchResults" class="max-h-96 overflow-y-auto"></div>
                </div>
            </div>
  ...
  <section class="flex flex-row h-screen">
    ...
    <section class="basis-1/6 border-r border-gray-100">
      <aside class="h-screen bg-white flex flex-col justify-between">
          <nav class="mt-5 px-2">
              ...
              <a
                ...
                class="mt-1 group flex items-center px-2 py-2 text-base font-medium rounded-md
                    text-gray-600 {{ Request::routeIs('borrow-records.*') ? 'bg-green-900 text-white' : 'hover:bg-gray-50 hover:text-gray-900' }} siemreap-regular"
            >
  ...
  <script>
    const searchRoutes = {
    dashboard: {
        keywords: ['dashboard', 'កម្រងទិន្នន័យ'],
        route: '/dashboard',
        display: 'Dashboard'
    },
    usersList: {
        keywords: ['user', 'member', 'សមាជិក', 'list'],
        route: '/users',
        display: 'Users List'
    },
    createUser: {
        keywords: ['create user', 'add user', 'បន្ថែមសមាជិក'],
        route: '/users/create',
        display: 'Create User'
    },
    booksList: {
        keywords: ['book', 'title', 'សៀវភៅ', 'list'],
        route: '/books',
        display: 'Books List'
    },
    createBook: {
        keywords: ['create book', 'add book', 'បន្ថែមសៀវភៅ'],
        route: '/books/create',
        display: 'Create Book'
    },
    borrowRecords: {
        keywords: ['borrow', 'check', 'record', 'borrow-records', 'ឆែកសៀវភៅ'],
...
function performGlobalSearch(query) {
    ...
    for (const key in searchRoutes) {
        const { keywords, route, display } = searchRoutes[key];
        if (keywords.some(word => word.includes(lowerQuery))) {
            foundAny = true;
            const item = document.createElement('div');
            item.className = 'p-2 hover:bg-gray-100 cursor-pointer rounded';
            item.textContent = display;
            item.onclick = () => window.location.href = route;
    ...
    if (!foundAny) {
        ...
        noResult.textContent = 'មិនមានលទ្ធផល';
